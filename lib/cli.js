...
function parseArgs(args) {
    ...
    args.forEach((arg, index) => {
        ...
        if (arg === '--help') {
            ...
            options.dryRun = true;
        } else if (arg === '--dry-run-auto') {
            options.dryRun = true;
            options.dryRunAuto = true;
        } else if (arg === '--tmdb-info') {
            options.tmdbInfoOnly = true;
        } else if (arg === '--no-eject') {
            options.ejectOnComplete = false;
        } else if (arg === '--start-episode' && args[index + 1]) {
            options.startEpisode = parseInt(args[index + 1], 10);
            consumedIndices.add(index + 1);
