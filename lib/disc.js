 ...
 * @param {boolean} isDryRun - Whether this is a dry run (adds DRYRUN_ prefix)
 * @returns {string} Output directory path
 */
function setDefaultOutputDir(volumeName, outputDir = null, isDryRun = false) {
    if (outputDir) {
    ...
    // Add _DRYRUN suffix for dry-run folders so they're easy to identify and delete (keeps chronological ordering)
    const suffix = isDryRun ? '_DRYRUN' : '';
    const newOutputDir = path.join(process.cwd(), `${safeName}_${timestamp}${suffix}`);
