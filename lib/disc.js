...
/**
 * Run lsdvd scan and extract track count and durations
 * @param {string} dvdSource - The DVD device path
 * @returns {Object|null} { trackCount, tracks } or null if lsdvd unavailable/failed
 */
function runLsdvdScan(dvdSource) {
    ...
    try {
        ...
        getLogger().debug(`[lsdvd] Found ${trackCount} tracks`);
 ...
 * @returns {Promise<{numTitles: number, titleDurations: Object, unrippableTracks: Array, scanWarning: string|null}>}
 */
async function scanDiscWithValidation(dvdSource, volumeName, options = {}) {
    ...
    while (retryCount <= maxRetries) {
        ...
        if (comparison.match) {
            // Scans agree - proceed with confidence
            return {
                ...handbrakeResult,
                scanWarning: null,
    ...
    // Show final "What's on the menu" since the retry scan was quiet
    if (lastHandbrakeResult && lastHandbrakeResult.titleDurations) {
        const finalDurations = lastHandbrakeResult.titleDurations;
        ...
        console.log('📋 What\'s on the menu (after retry):');
    ...
    console.log('   ⚠️  SCAN WARNING');
    console.log('   ━'.repeat(30));
    console.log(`   HandBrake found: ${comparison.handbrakeCount} tracks`);
    console.log(`   lsdvd found:     ${comparison.lsdvdCount} tracks`);
    console.log('');
    console.log('   This can happen with scratched discs or copy protection.');
    console.log('   Consider using --interactive mode for manual track selection.');
    console.log('   ━'.repeat(30));
    console.log('');
...
async function scanDisc(dvdSource, volumeName, options = {}) {
    process.stdout.write('🍊 Inspecting the fruit...\n');
    ...
    const scanResult = await new Promise((resolve, reject) => {
        ...
        const processData = (data) => {
            ...
            if (totalMatch) {
                ...
                console.log(`   Found ${totalTitles} segment${totalTitles > 1 ? 's' : ''} - checking freshness...`);
    ...
    if (code === 0 || output.includes('scan: DVD has')) {
        ...
        if (match) {
            ...
            console.log(`✓ Found ${numTitles} ripe title${numTitles > 1 ? 's' : ''}`);
            console.log('');
