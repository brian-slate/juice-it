...
/**
 * Display TMDB information for debugging
 * @param {Object} metadata - Metadata from TMDB lookup
 * @param {Function} printSeparator - Function to print separator lines
 */
function displayTmdbInfo(metadata, printSeparator) {
    console.log('');
    printSeparator();
    console.log('  📊 TMDB Information');
    printSeparator();

    if (!metadata || metadata.type === 'disc' || metadata.type === 'cancel') {
        console.log('\n  ⚠️  No TMDB data available');
        console.log('     Metadata lookup was cancelled or not performed.\n');
        return;
    }

    console.log('');
    console.log(`  Show:    ${metadata.name}`);
    if (metadata.id) {
        console.log(`  TMDB ID: ${metadata.id}`);
    }
    if (metadata.year) {
    ...
    if (metadata.type === 'tv' && metadata.episodes) {
        ...
        // Show multi-season info if applicable
        if (metadata.isMultiSeason && metadata.seasonInfos) {
            console.log(`  Seasons:         ${metadata.seasons.join(', ')}`);
            console.log(`  Total Episodes:  ${metadata.episodes.length} (aggregated from ${metadata.seasonInfos.length} seasons)`);
            console.log('');
            metadata.seasonInfos.forEach(info => {
                console.log(`    • Season ${info.seasonNumber}: ${info.episodeCount} episodes`);
            });
        } else {
            console.log(`  Season:          ${metadata.season || '?'}`);
            console.log(`  Total Episodes:  ${metadata.episodes.length}`);
        }

        // Calculate episode runtime statistics
        const runtimes = metadata.episodes.map(ep => ep.runtime).filter(r => r);
        if (runtimes.length > 0) {
            ...
            console.log(`  Avg Runtime:     ${avgRuntime} min (range: ${minRuntime}-${maxRuntime} min)`);
        }
        ...
        metadata.episodes.forEach((ep) => {
            ...
            const runtime = ep.runtime ? `${ep.runtime} min` : '? min';
            // Show source season for multi-season aggregations
            const seasonInfo = metadata.isMultiSeason && ep._sourceSeasonNumber
                ? ` [from S${ep._sourceSeasonNumber}E${ep._originalEpisodeNumber}]`
                : '';
            console.log(`  E${epNum}: ${ep.name} (${runtime})${seasonInfo}`);
        });
...
async function ripAllTracks(options) {
    ...
    let autoModeWarnings = []; // Warnings from automatic mode (let for reassignment)
    let aiMappingResult = null; // AI mapping result (needed for re-search flows)

    try {
        const volumeName = discModule.getVolumeName(options.dvdSource);

        // Set default output directory before any operations (with DRYRUN_ prefix if dry-run mode)
        options.outputDir = discModule.setDefaultOutputDir(volumeName, options.outputDir, options.dryRun || false);
        ...
        // If tmdb-info mode, display episode data and exit
        if (options.tmdbInfoOnly) {
            displayTmdbInfo(metadata, printSeparatorFn);
            return { successCount: 0, skippedTracks, mappingSkippedTracks };
        }
        ...
        // Log is already initialized at the start - no need to reinitialize
        ...
        if (proposedMappings.length === 0) {
            ...
            if (config.openaiApiKey && titleDurations && metadata.type === 'tv') {
                ...
                // Show start episode override if provided
                if (options.startEpisode) {
                    console.log(`  ✓ Using --start-episode ${options.startEpisode}`);
                    logFn(`Start episode override: ${options.startEpisode}`);
                }
                ...
                while (!aiMappingResult && !useSequentialMapping) {
                    aiMappingResult = await aiModule.aiMapTracks(titleDurations, metadata, lsdvdMetadata, unrippableTracks || [], metadata.discNumber, volumeName, options.startEpisode || null, enhancedTrackData);
            ...
            if (aiMappingResult) {
                proposedMappings = mappingModule.buildProposedMappingsFromAI(
                    ...
                    { mainOnly: options.mainOnly, ripAll: options.ripAll, unrippableTracks }
                );
            } else {
                proposedMappings = mappingModule.buildSequentialMappings(
                    metadata, numTitles, titleDurations,
                    { mainOnly: options.mainOnly, ripAll: options.ripAll, unrippableTracks, mainFeatureTrack }
                );
            }
        ...
        if (skipReview) {
            ...
            mappingsToRip = await interactiveModule.reviewAndMapEpisodesBeforeRip(
                ...
                { interactive: options.interactive, verbose: options.verbose }
            );
        ...
        // Loop to allow re-search if needed
        let warningsHandled = false;
        while (!warningsHandled && autoModeWarnings.length > 0 && !options.rawMode) {
            console.log('');
            printSeparatorFn();
            ...
            // Show contextual tip based on whether user provided a search query
            if (!options.searchQuery) {
                // No search query provided - show examples
                console.log('  💡 Tip: Provide a specific search query for better results');
                console.log('');
                console.log('     TV Show examples:');
                console.log('       juice-it "Ed, Edd n Eddy season 2"');
                console.log('       juice-it "The Office US s03"');
                console.log('       juice-it "Breaking Bad s5 & 6"');
                console.log('');
                console.log('     Movie examples:');
                console.log('       juice-it "Avatar 2009"');
                console.log('       juice-it "The Matrix 1999"');
                console.log('');
            } else {
                // Search query was provided but didn't work well - suggest trying a different one
                console.log('  💡 Tip: Current search may not be accurate - try a different query');
                console.log(`     You searched for: "${options.searchQuery}"`);
                console.log('');
            }
            ...
            if (options.interactive) {
                ...
                try {
                    // Change menu text based on whether user provided a query
                    const researchMenuText = options.searchQuery
                        ? '🔍 Re-run with a better search query'
                        : '🔍 Enter a search query for better results';

                    const warningChoices = [
                        { name: '🍊 Continue with ripping', value: 'continue' },
                        { name: researchMenuText, value: 'research' },
                    ...
                    if (warningChoice === 'cancel') {
                        ...
                        console.log('');
                        return { successCount: 0, skippedTracks, mappingSkippedTracks };
                    } else if (warningChoice === 'continue') {
                        // User chose to continue despite warnings
                        warningsHandled = true;
                    } else if (warningChoice === 'research') {
                        // Re-run with a better search query - show contextual tip
                        console.log('');

                        if (!options.searchQuery) {
                            // No query was provided - show full examples
                            console.log('  💡 Tip: Provide a specific search query for better results');
                            console.log('');
                            console.log('     TV Show examples:');
                            console.log('       "Ed, Edd n Eddy season 2"');
                            console.log('       "The Office US s03"');
                            console.log('       "Breaking Bad s5 & 6"');
                            console.log('');
                            console.log('     Movie examples:');
                            console.log('       "Avatar 2009"');
                            console.log('       "The Matrix 1999"');
                            console.log('');
                        } else {
                            // Query was provided - suggest refining it
                            console.log('  💡 Tip: Try a more specific or different search query');
                            console.log(`     Previous search: "${options.searchQuery}"`);
                            console.log('');
                            console.log('     Tips: Add year for movies, season number for TV shows');
                            console.log('');
                        }
                        ...
                        if (newQuery && newQuery.trim()) {
                            ...
                            // Update options with new search query
                            options.searchQuery = newQuery.trim();

                            // Restart metadata lookup with new query
                            metadata = await metadataModule.lookupMetadata(
                                volumeName, numTitles, titleDurations,
                                { searchQuery: newQuery.trim(), interactive: false },
                                lsdvdMetadata
                            );

                            if (!metadata) {
                                console.log('');
                                console.log('  ℹ️  Search cancelled or failed.');
                                console.log('');
                                return { successCount: 0, skippedTracks, mappingSkippedTracks };
                            }

                            // Re-run AI mapping with new metadata
                            console.log('');
                            console.log('  🤖 Re-analyzing tracks with new metadata...');
                            console.log('');

                            aiMappingResult = await aiModule.aiMapTracks(
                                titleDurations, metadata, volumeName,
                                { startEpisode: options.startEpisode, enhancedTrackData, lsdvdMetadata }
                            );

                            if (!aiMappingResult) {
                                console.error('\n❌ AI mapping failed after re-search\n');
                                return { successCount: 0, skippedTracks, mappingSkippedTracks };
                            }

                            // Rebuild proposed mappings
                            proposedMappings = mappingModule.buildProposedMappingsFromAI(
                                aiMappingResult, metadata, numTitles, titleDurations,
                                { mainOnly: options.mainOnly, ripAll: options.ripAll, unrippableTracks }
                            );
                            ...
                            console.log('');
                            console.log('  🔍 Validating new mappings...');
                            console.log('');

                            const reValidation = await aiModule.aiValidateMappings(
                                proposedMappings, titleDurations, metadata,
                                { searchQuery: newQuery.trim(), extractedInfo: { confidence: 1.0 } }
                            );

                            autoModeWarnings = reValidation?.concerns
                                ?.filter(c => c.severity === 'medium' || c.severity === 'high')
                                ?.map(c => c.message) || [];

                            // Update mappingsToRip with new mappings
                            mappingsToRip = proposedMappings.filter(m => m.status !== 'skip' && m.status !== 'unrippable');

                            // Loop will check if there are still warnings and show them again, or exit loop if none
                        } else {
                            console.log('');
                            console.log('  ℹ️  No query entered - returning to warnings menu.');
                            console.log('');
                            // Loop will show warnings menu again
                        }
                        ...
                        mappingsToRip = await interactiveModule.reviewAndMapEpisodesBeforeRip(
                            proposedMappings, metadata, volumeName, baseFileName,
                            { interactive: true, verbose: options.verbose }
                        );
                    ...
                    // User cancelled prompt (Ctrl+C) or error occurred
                    console.log('');
                    if (e && e.message && !e.message.includes('User force closed')) {
                        // Real error, not just Ctrl+C
                        console.log('  ❌ Error during processing:');
                        console.log(`     ${e.message}`);
                        logger.error(`Error in warnings menu: ${e.message}`);
                        if (options.verbose) {
                            console.log(e.stack);
                        }
                    } else {
        ...
        } // end while loop for warnings

        // Always show "Ready to Juice?" confirmation before starting (unless raw mode or dry-run-auto)
        // Loop to allow going back from interactive mode
        let confirmationComplete = false;
        while (!options.rawMode && !options.dryRunAuto && !confirmationComplete) {
            const confirmation = await interactiveModule.confirmBeforeRip(
                mappingsToRip, metadata, { searchQuery: options.searchQuery, interactive: options.interactive, rawMode: options.rawMode, dryRun: options.dryRun }
            );
            ...
            if (confirmation.action === 'cancel') {
                ...
                // Check if user wants to go back to confirmation menu
                if (mappingsToRip && mappingsToRip._action === 'back') {
                    console.log('');
                    continue; // Go back to confirmation menu
                }
                ...
                if (newMetadata && newMetadata.type !== 'cancel') {
                    ...
                    if (metadata.type === 'tv' && config.openaiApiKey) {
                        ...
                        proposedMappings = mappingModule.buildSequentialMappings(
                            metadata, numTitles, titleDurations,
                            { mainOnly: options.mainOnly, ripAll: options.ripAll, unrippableTracks, mainFeatureTrack: mappingModule.findMainFeatureTrack(titleDurations) }
                        );
                    }
                    ...
                    const reConfirmation = await interactiveModule.confirmBeforeRip(
                        mappingsToRip, metadata, { searchQuery: confirmation.newQuery, interactive: options.interactive, rawMode: options.rawMode, dryRun: options.dryRun }
                    );
                ...
                confirmationComplete = true;
            } else if (confirmation.action === 'rip-real') {
                // User selected "rip for real" in dry-run mode - switch to real encoding
                confirmationComplete = true;
            } else if (confirmation.action === 'rip') {
                // User selected "rip" (either normal mode, or "continue dry-run" in dry-run mode)
                confirmationComplete = true;
            } else {
            ...
            // If user was in dry-run mode and selected "rip for real", turn off dry-run
            if (confirmation.action === 'rip-real' && options.dryRun) {
                console.log('');
                console.log('  💡 Switching from dry-run to real encoding...');
                console.log('');

                // Turn off dry-run mode
                options.dryRun = false;

                // Update output directory to remove _DRYRUN suffix
                const oldOutputDir = options.outputDir;
                options.outputDir = oldOutputDir.replace(/_DRYRUN$/, '');

                // If directory name changed, create new directory and move log file
                if (oldOutputDir !== options.outputDir) {
                    const fs = require('fs');
                    ...
                    if (logFilePath && fs.existsSync(logFilePath)) {
                        const logFileName = require('path').basename(logFilePath);
                        const newLogPath = require('path').join(options.outputDir, logFileName);
                        fs.renameSync(logFilePath, newLogPath);
                        logFilePath = newLogPath;
                        logFn(`Moved to real encoding - output directory: ${options.outputDir}`);
                    }
        ...
        printSeparatorFn();
        if (options.dryRun) {
            console.log('  🧪 DRY-RUN MODE - Preview only (no files created)');
        } else {
        ...
        for (const m of mappingsToRip) {
            ...
            // In dry-run mode, create stub files instead of ripping
            if (options.dryRun) {
                const stubFilePath = path.join(options.outputDir, finalFileName);
                try {
                    // Create empty stub file
                    fs.writeFileSync(stubFilePath, '');
                    console.log(`  ✓ Track ${titleNumber}: ${finalFileName}`);
                    successCount++;
                } catch (error) {
                    console.log(`  ✗ Track ${titleNumber}: Failed to create stub file`);
                    logFn(`Failed to create stub file: ${error.message}`);
                }
                continue;
            }
        ...
        // Dry-run completion notice
        if (options.dryRun) {
            console.log('  ℹ️  DRY-RUN COMPLETE');
            console.log('     No actual encoding was performed - only stub files were created');
            console.log('     Run without --dry-run to actually rip the tracks:');
            console.log('');
            console.log(`     juice-it${options.searchQuery ? ` "${options.searchQuery}"` : ''}`);
            console.log('');
        }
        ...
        // Show recommended folder structure if we have metadata with TMDB ID
        ...
        if (plexPath && successCount > 0) {
            ...
            console.log('');
            if (metadata.tmdbId) {
                console.log(`  The {tmdb-${metadata.tmdbId}} tag ensures exact matching in media servers.`);
            }
            console.log('');
        }
        ...
        if (mappingSkippedTracks.length > 0 && !options.includeExtras) {
            ...
            mappingSkippedTracks.forEach(({ track, duration, reason }) => {
                console.log(`    • Track ${track} (${duration} min): ${reason || 'menu/extra'}`);
            });
            console.log('');
            if (options.mainOnly) {
                ...
                console.log('');
            } else if (!options.ripAll) {
                console.log('  💡 To also rip Play All compilations, use --rip-all:');
                console.log('');
                console.log('     juice-it --rip-all');
                console.log('');
            }
        ...
        // Show success, warning, or error ending message
        if (successCount > 0) {
            const hasWarnings = autoModeWarnings.length > 0 || skippedTracks.length > 0;

            if (!hasWarnings) {
                // Perfect success
                console.log('');
                console.log('  🍊 All tracks juiced successfully!');
                console.log('');
            } else {
                // Success with warnings
                console.log('');
                console.log('  🍊 Rip completed with warnings - please review your files.');
                console.log('');
            }
        } else {
            // Complete failure
            console.log('');
            console.log('  🍌 Whoops! Rip failed - check the errors above.');
            console.log('');
        }
