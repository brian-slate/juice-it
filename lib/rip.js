...
async function ripAllTracks(options) {
    ...
    try {
        ...
        // Pass volumeName for disc pattern extraction (S02D03, DISC_TWO, etc.)
        ...
        if (options.rawMode) {
            ...
            if (!hasUserQuery && !isInteractive) {
                // Naked invocation: try best-effort auto-detection from disc metadata
                // If AI is confident (>= 0.7), proceed with auto lookup
                // Otherwise, fall back to guided selection

                // Try AI-based query extraction from disc title
                if (config.openaiApiKey && lsdvdMetadata && lsdvdMetadata.discTitle) {
                    console.log('');
                    ...
                    if (extractedInfo && extractedInfo.confidence >= 0.7) {
                        ...
                        // Low confidence - show what we found but fall back to guided selection
                        console.log(`   ⚠️  Low confidence (${Math.round(extractedInfo.confidence * 100)}%) - falling back to selection menu`);
                        if (extractedInfo.searchQuery) {
                            console.log(`   Best guess: "${extractedInfo.searchQuery}"`);
                        }
                        logFn(`Best-effort auto-detection: Low confidence (${extractedInfo.confidence}), falling back to guided selection`);
                    }
                }

                // Fall back to guided selection if auto-detection failed or low confidence
                if (!metadata) {
        ...
        if (autoModeWarnings.length > 0 && !options.rawMode) {
            ...
            if (options.interactive) {
                ...
                try {
                    ...
                    const warningChoice = await warningPrompt.run();
        ...
        if (!options.rawMode) {
                ...
                if (newMetadata && newMetadata.type !== 'cancel') {
                    ...
                    if (metadata.type === 'tv' && config.openaiApiKey) {
                        const newMappingResult = await aiModule.aiMapTracks(
                            ...
                            metadata.discNumber || null, volumeName, null
                        );
                        if (newMappingResult) {
                            proposedMappings = mappingModule.buildProposedMappingsFromAI(
            ...
            // 'rip' action falls through to start ripping
        }
