...
/**
 * Aggregate episodes from multiple TV seasons
 *
 * @param {number} tvShowId - TMDB TV show ID
 * @param {number[]} seasons - Array of season numbers to fetch
 * @returns {Promise<Object>} Object with aggregated episodes and season info
 */
async function aggregateMultiSeasonEpisodes(tvShowId, seasons) {
    const { getTVSeasonDetails } = getTmdb();
    const logger = getLogger();

    logger.debug(`Aggregating episodes from seasons: ${seasons.join(', ')}`);

    const allEpisodes = [];
    const seasonInfos = [];

    for (const seasonNum of seasons) {
        const seasonDetails = await getTVSeasonDetails(tvShowId, seasonNum);
        if (seasonDetails && seasonDetails.episodes) {
            // Add season marker to each episode
            const episodesWithSeason = seasonDetails.episodes.map(ep => ({
                ...ep,
                _sourceSeasonNumber: seasonNum
            }));
            allEpisodes.push(...episodesWithSeason);
            seasonInfos.push({
                seasonNumber: seasonNum,
                episodeCount: seasonDetails.episodes.length,
                name: seasonDetails.name
            });
            logger.debug(`  Season ${seasonNum}: ${seasonDetails.episodes.length} episodes`);
        } else {
            logger.debug(`  Season ${seasonNum}: no episodes found`);
        }
    }

    // Re-number episodes sequentially starting from 1
    allEpisodes.forEach((ep, idx) => {
        ep._originalEpisodeNumber = ep.episode_number;
        ep._originalSeasonNumber = ep.season_number;
        ep.episode_number = idx + 1;
        ep.season_number = seasons[0]; // Use first season as the "primary" season
    });

    logger.debug(`Total aggregated episodes: ${allEpisodes.length}`);
...
async function lookupMetadata(volumeName, numTitles, trackDurations = null, searchOptions = {}, lsdvdMetadata = null) {
    ...
    if (searchOptions.searchQuery) {
        ...
        if (extractedInfo && extractedInfo.searchQuery) {
            ...
            // Check if this is a generic/unknown title from auto-detection
            const isGeneric = /^unknown$/i.test(cleanName) || cleanName.toLowerCase() === 'untitled';
            if (isGeneric && !searchOptions.searchQuery) {
                console.log(`   ℹ️  No title detected - using generic track names`);
            } else {
                console.log(`   Using title: "${cleanName}"`);
            }
            ...
            // Skip if: user provided single season, user provided multiple seasons, or interactive mode is disabled
            const hasSeasonInfo = extractedInfo.season || (extractedInfo.seasons && extractedInfo.seasons.length > 0);
            const allowInteractive = searchOptions.interactive !== false;

            if (extractedInfo.disc && !hasSeasonInfo && extractedInfo.isTV && allowInteractive) {
        ...
        if (aiSelection && aiSelection.selectedId) {
            ...
            if (selectedResult && shouldAutoSelect) {
                ...
                if (aiSelection.selectedType === 'tv') {
                    ...
                    // Check if multi-season query (e.g., "season 5 & 6")
                    if (extractedInfo?.seasons && extractedInfo.seasons.length > 1) {
                        console.log(`   Fetching episodes from seasons: ${extractedInfo.seasons.join(', ')}...`);
                        const aggregated = await aggregateMultiSeasonEpisodes(selectedResult.id, extractedInfo.seasons);
                        const showOverview = await getTVShowDetails(selectedResult.id);

                        console.log(`\n   ✨ Auto-selected: ${selectedResult.name} - Seasons ${extractedInfo.seasons.join(' & ')}${confidenceStr}`);
                        console.log(`   Episodes aggregated: ${aggregated.episodes.length} total`);
                        aggregated.seasonInfos.forEach(info => {
                        ...
                        return {
                            ...
                            year: showYear,
                            season: extractedInfo.seasons[0], // Primary season for naming
                            seasons: extractedInfo.seasons, // Track all seasons
                            episodes: aggregated.episodes,
                            showOverview: showOverview || null,
                            aiSelected: true,
                            discNumber: extractedInfo?.disc || null,
                            isMultiSeason: true,
                            seasonInfos: aggregated.seasonInfos
                        };
                    }
    ...
    try {
        ...
        if (choice === 'accept' && selectedResult) {
            if (aiSelection.selectedType === 'tv') {
                ...
                // Check if multi-season query
                if (extractedInfo?.seasons && extractedInfo.seasons.length > 1) {
                    console.log(`   Fetching episodes from seasons: ${extractedInfo.seasons.join(', ')}...`);
                    const aggregated = await aggregateMultiSeasonEpisodes(selectedResult.id, extractedInfo.seasons);
                    const showOverview = await getTVShowDetails(selectedResult.id);

                    console.log(`\n   ✓ Accepted: ${selectedResult.name} - Seasons ${extractedInfo.seasons.join(' & ')}\n`);
                    console.log(`   Episodes aggregated: ${aggregated.episodes.length} total`);
                    aggregated.seasonInfos.forEach(info => {
                        console.log(`     • Season ${info.seasonNumber}: ${info.episodeCount} episodes`);
                    });
                    ...
                    return {
                        ...
                        name: selectedResult.name,
                        year: showYear,
                        season: extractedInfo.seasons[0],
                        seasons: extractedInfo.seasons,
                        episodes: aggregated.episodes,
                        showOverview: showOverview || null,
                        discNumber: extractedInfo?.disc || null,
                        isMultiSeason: true,
