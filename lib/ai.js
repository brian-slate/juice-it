 ...
 * @param {Array|null} enhancedTrackData - Enhanced track data with chapters from buildEnhancedTrackData()
 * @returns {Promise<Object|null>} Mapping result or null
 */
async function aiMapTracks(trackDurations, metadata, lsdvdMetadata = null, unrippableTracks = [], discNumber = null, volumeName = null, startEpisodeOverride = null, enhancedTrackData = null) {
    try {
        ...
        getLogger().debug(`[AI] Enhanced track data: ${enhancedTrackData ? `${enhancedTrackData.length} tracks with chapter info` : 'not available'}`);
        ...
        logToFile(`AI: Enhanced track data - ${enhancedTrackData ? `${enhancedTrackData.length} tracks` : 'not available'}`);
