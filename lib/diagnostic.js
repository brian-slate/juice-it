...
async function runDiagnosticMode(options = {}) {
    ...
    const lsdvdMetadata = discModule.getLsdvdMetadata(dvdSource, volumeName);
    ...
    // Use disc module scanDiscWithValidation() - returns structured data with dual-tool validation
    const scanResult = await discModule.scanDiscWithValidation(dvdSource, volumeName, { verbose: options.verbose });
    ...
    // Show scan validation results in diagnostic mode
    if (scanResult.scanWarning) {
        deps.log('');
        deps.log('  ⚠️  SCAN VALIDATION WARNING:');
        deps.log(`      ${scanResult.scanWarning}`);
    }
    ...
    // Note: enhancedTrackData not available in diagnostic mode (would need full scan), pass null
    const aiMappingResult = await aiModule.aiMapTracks(trackDurations, metadata, lsdvdMetadata, unrippableTracks || [], metadata.discNumber, volumeName, null, null);
