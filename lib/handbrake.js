...
/**
 * Helper function to check if a DVD device exists
 * @param {string} device - Device path (e.g., /dev/disk4)
 * @returns {boolean} True if device exists
 */
function isDvdDevicePresent(device) {
    try {
        const result = deps.spawnSync('diskutil', ['list', device], { timeout: 3000 });
...
/**
 * Helper function to find DVD device path from diskutil list
 * @returns {string|null} DVD device path (e.g., /dev/disk4) or null if not found
 */
function findDvdDevice() {
    try {
        const result = deps.spawnSync('diskutil', ['list'], { encoding: 'utf-8', timeout: 3000 });
        if (result.status !== 0 || !result.stdout) return null;

        const lines = result.stdout.split('\n');
        // Look for external physical disks without a proper partition scheme (typical of DVDs)
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
            if (line.includes('(external, physical)') && line.includes('/dev/disk')) {
        ...
        getLogger().debug(`Error finding DVD device: ${error.message}`);
        return null;
    }
}

/**
 * Eject the disc from the DVD drive with verification
 * @param {string} dvdSource - The DVD device path (optional)
 * @returns {boolean} True if eject was successful AND verified
 */
function ejectDisc(dvdSource = null) {
    getLogger().debug('Attempting to eject disc...');

    // Auto-detect DVD device if not provided
    let devicePath = dvdSource;
    if (!devicePath || !devicePath.startsWith('/dev/')) {
        devicePath = findDvdDevice();
        if (devicePath) {
            getLogger().debug(`Auto-detected DVD device: ${devicePath}`);
        }
    }

    // Check if device exists before attempting eject
    if (devicePath && !isDvdDevicePresent(devicePath)) {
        getLogger().debug('No DVD device found to eject');
        return true; // Nothing to eject
    }

    // Method 1: drutil eject (most forceful - direct to drive firmware)
    getLogger().debug('Trying drutil eject...');
    const drutilResult = deps.spawnSync('drutil', ['eject'], { timeout: 5000 });

    if (drutilResult.status === 0) {
        // Wait briefly for physical ejection to complete
        const start = Date.now();
        while (Date.now() - start < 1500) {
            // Synchronous wait
        }

        // Verify ejection if we have a device path
        if (devicePath) {
            if (!isDvdDevicePresent(devicePath)) {
                getLogger().debug('Ejection verified via drutil');
                return true;
            } else {
                getLogger().debug('drutil returned success but disc still present');
            }
        } else {
            // No device path to verify, trust the exit code
            getLogger().debug('Ejected via drutil (no verification)');
            return true;
        }
    } else {
        getLogger().debug('drutil eject failed');
    }

    // Method 2: diskutil eject with device path
    if (devicePath) {
        getLogger().debug(`Trying diskutil eject ${devicePath}...`);
        const diskutilResult = deps.spawnSync('diskutil', ['eject', devicePath], { timeout: 5000 });
        ...
        if (diskutilResult.status === 0) {
            ...
            if (!isDvdDevicePresent(devicePath)) {
                getLogger().debug('Ejection verified via diskutil');
                return true;
            } else {
                getLogger().debug('diskutil returned success but disc still present');
            }
        } else {
            getLogger().debug('diskutil eject failed');
        }
    }

    getLogger().debug('All eject methods failed or could not be verified');
    return false;
}
...
function ripDvd(options, onProgress, trackNum, totalTracks) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // Dry-run mode: create stub file for reference
        if (dryRun) {
            logToFile(`[DRY-RUN] Would rip track ${titleNumber} to: ${outputFileName}.mp4`);

            // Create a small placeholder file so users can see what would have been created
            try {
                const stubContent = `DRY RUN PLACEHOLDER\n\nThis is a placeholder file created during dry-run mode.\n\nOriginal track: ${titleNumber}\nOutput file: ${outputFileName}.mp4\n`;
                deps.fs.writeFileSync(outputFilePath, stubContent, 'utf8');
                logToFile(`[DRY-RUN] Created stub file: ${outputFilePath}`);
            } catch (err) {
                logToFile(`[DRY-RUN] Warning: Could not create stub file: ${err.message}`);
            }

            // Immediately resolve without simulating progress
            resolve();
            return;
        }
