...
/**
 * Eject the disc from the DVD drive (no wait for reinsertion)
 * @param {string} dvdSource - The DVD device path (optional, for diskutil method)
 * @returns {boolean} True if eject was successful
 */
function ejectDisc(dvdSource = null) {
    getLogger().debug('Ejecting disc...');
    ...
    // Method 2: diskutil eject (if we have a source path)
    if (dvdSource) {
    ...
    getLogger().debug('Eject failed - disc may already be ejected or drive is busy');
    return false;
}
