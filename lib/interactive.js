 ...
 * @param {Object} trackDurations - Track durations
 * @param {Object} lsdvdMetadata - lsdvd metadata
 * @param {Object} options - Options {searchQuery, unrippableTracks, enhancedTrackData}
...
async function interactiveStepThrough(volumeName, numTitles, trackDurations, lsdvdMetadata, options = {}) {
    ...
    const enhancedTrackData = options.enhancedTrackData || null;
 ...
 * @param {string} volumeName - DVD volume name
 * @param {string} baseFileName - Base filename for Plex
 * @param {Object} options - Options {interactive, verbose}
...
async function reviewAndMapEpisodesBeforeRip(proposedMappings, contentMetadata, volumeName, baseFileName, options = {}) {
    const { interactive = false, verbose = false } = options;
    ...
    while (true) {
        ...
        try {
            ...
            if (choice === '← Back to confirmation menu') {
                // Return special marker to indicate user wants to go back
                return { _action: 'back' };
            } else if (choice === 'Edit Track Mapping') {
...
async function confirmBeforeRip(proposedMappings, contentMetadata, options = {}) {
    ...
    console.log('━'.repeat(60));
    if (options.dryRun) {
        console.log('  🍊 Ready to Juice? (DRY-RUN MODE)');
    } else {
    ...
    // Show all files to be ripped
    if (toRip.length > 0) {
        console.log('  Files to create:');
        for (const m of toRip) {
            const filename = m.proposedName || m.status || 'unknown';
            // Truncate long filenames for display, but show enough to identify
            const displayName = filename.length > 65 ? filename.substring(0, 62) + '...' : filename;
        ...
        // Dry-run notice
        if (options.dryRun) {
            console.log('  ℹ️  DRY-RUN MODE: No actual encoding will happen');
            console.log('     Small stub files will be created for reference');
    ...
    // In dry-run mode, offer both options
    if (options.dryRun) {
        choices.push({ name: '🍊 Juice It! (rip tracks for real)', value: 'rip-real' });
        choices.push({ name: '🧪 Continue with dry-run (stub files only)', value: 'rip' });
    } else {
        choices.push({ name: '🍊 Juice it!', value: 'rip' });
    }

    choices.push({ name: '✏️  Edit mappings', value: 'edit' });
