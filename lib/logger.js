...
class Logger {
    ...
    initFileLogging(outputDir, identifier) {
        // If file logging is already initialized, don't create a duplicate
        if (this.fileStream && this.filePath) {
            this.debug('File logging already initialized - reusing existing log file');
    ...
    _log(level, message, options = {}) {
        ...
        if (this.fileStream && levelConfig.priority >= LOG_LEVELS[this.fileLevel].priority) {
            const timestamp = new Date().toISOString();
            this._writeToFile(`[${timestamp}] [${levelConfig.label}] ${prefix}${message}`);
        }
