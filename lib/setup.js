...
async function runSetup() {
    ...
    console.log('  ✅ FREE to use - no payment required');
    console.log('  ✅ A demo key is included - you can skip this step');
    console.log('');
    console.log('  Want your own key? (Recommended for heavy use)');
    console.log('    1. Sign up at https://www.themoviedb.org/signup');
    console.log('    2. Go to https://www.themoviedb.org/settings/api');
    console.log('    3. Request an API key (choose "Developer")');
    console.log('');

    const tmdbPrompt = new deps.Input({
        message: 'Enter your TMDB API key (or press Enter to use free demo key):',
        validate(_value) {
    ...
    try {
        ...
        if (tmdbApiKey && tmdbApiKey.trim().length > 0) {
            ...
            if (isTmdbValid) {
                ...
                console.log('❌ Invalid TMDB API key. Using demo key instead.');
                settings.tmdbApiKey = 'REMOVED_API_KEY';
            }
        } else {
            // Use demo key
            console.log('');
            console.log('✅ Using free demo TMDB key (this is fine for most users)');
            settings.tmdbApiKey = 'REMOVED_API_KEY';
        }
